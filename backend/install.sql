...
begin
	begin try
		drop database [IZVRSITELJSKA_KUCA];
	end try begin catch
		select 'Tabela ne postoji ili je nastala greska pri brisanju'
	end catch
	begin try
		drop login DB_ADMIN;
		drop user DB_ADMIN;
		...
		select 'Korisnik/login ne postoji ili ne nastala greska pri brisanju'
	end catch
	create database [IZVRSITELJSKA_KUCA];
end
go
...
begin
	...
	alter table M_KOLONE
	add constraint FK_M_KOLONE_META_TABELE
	foreign key (M_TABELE_ID)
	references M_TABELE (ID);
end
-- Deep breath
go
